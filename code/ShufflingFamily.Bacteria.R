...
input_dir <- "C:/Users/charmaine/OneDrive/Desktop/bacdive-client/nopool_family_level_output_bacteria"
output_dir <- "C:/Users/charmaine/OneDrive/Desktop/bacdive-clientshuffling_results_family_bacteria"  
...
#Helper functions for weighted mean, variance, and kurtosis
...
input_files <- list.files(input_dir, pattern="_family_with_cellsize\\.csv$", full.names=TRUE)

# Main loop over files
for (file in input_files) {
  # Extract host species name
  fname <- basename(file)
  host_species <- sub("_L7_family_with_cellsize\\.csv$", "", fname)
  ...
  for (sample_col in sample_cols) {
    ...
    # Subset data for this sample, remove NAs in GeoMeanVolume_family_median
    sub_dt <- dt[!is.na(GeoMeanVolume_family_median) & !is.na(get(sample_col)) & !is.na(get(rel_abund_col)),
                 .(family, GeoMeanVolume_family_median, rel_abund=get(rel_abund_col))]
    if (nrow(sub_dt) < 2) next  # Need at least 2 families for shuffling
    ...
    # Original statistics
    orig_mean <- weighted_mean(sub_dt$GeoMeanVolume_family_median, sub_dt$rel_abund)
    orig_var <- weighted_var(sub_dt$GeoMeanVolume_family_median, sub_dt$rel_abund)
    orig_kurt <- weighted_kurtosis(sub_dt$GeoMeanVolume_family_median, sub_dt$rel_abund)
    ...
    # Generate unique shuffles
    shuffles <- unique_shuffles(sub_dt$GeoMeanVolume_family_median, n_shuffles)
    ...
    orig_weighted <- rep(sub_dt$GeoMeanVolume_family_median, times=times_vec)
...
# Plot histogram
hist_pdf <- file.path(output_dir, "empirical_pvalue_histogram_family.pdf")
